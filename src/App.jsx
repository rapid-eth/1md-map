  ...
  useEffect(() => {
    const distances =
      ...
      eventsData.map(e => {
        const dist =
          viewport &&
          distanceMeters(viewport.latitude, viewport.longitude, e.lat, e.lon);
    ...
    return (
      hoveredObject && (
        <div className={"hover"} style={{ left: pointerX, top: pointerY }}>
          ...
          <h6>
            <a href={hoveredObject.link}>{hoveredObject.link}</a>
          </h6>
        </div>
      )
    );
      ...
      <div className="parent">
        <div className="leftChild">
          {clickedEvent ? (
            <EventCard
              event={clickedEvent}
              close={() => setClickedEvent(undefined)}
            />
          ) : (
            <div>
